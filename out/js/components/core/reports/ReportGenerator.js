  ...
  render() {
    ...
    if (!listOfChecks) {
      return React.createElement(
        "div",
        null,
        "No project data for checks found"
      );
    }
    if (!targetLang) {
      return React.createElement(
    ...
    for (let view in reportViews) {
      let viewResult;
      try {
        // in case their report view has errors
        viewResult = reportViews[view](0, 0);
      } catch (e) {
        continue;
      }
      if (viewResult) {
        reportHeaders.push(React.createElement(
          Col,
          { className: "pull-right", key: `0-0-${ view }`, xs: 6 },
          React.createElement(
            "span",
            null,
            viewResult
    ...
    for (let ch in targetLang) {
      ...
      for (let v in targetLang[ch]) {
        ...
        for (let view in reportViews) {
          ...
          if (viewResult) {
            reports.push(React.createElement(
              "span",
              { key: `${ ch }-${ v }-${ view }` },
              viewResult
            ));
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "small",
          null,
          "By " + authors + ", created on " + new Date().toDateString()
        )
      ),
      output
    );
...
module.exports = function (callback = err => {}) {
  ...
  fs.readFile("./src/js/components/core/reports/report-template.html", 'utf-8', (err, data) => {
    ...
    ReactDOM.render(React.createElement(Report, null), reportHTML.getElementsByTagName('div')[0]);
