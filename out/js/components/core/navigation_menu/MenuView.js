  ...
  class MenuView extends React.Component {
    ...
    goToNext() {
      ...
      if (this.currentCheckIndex >= this.state.checkObject.groups[this.currentGroupIndex].checks.length - 1) {
        ...
        // if we still have more in the group
    ...
    render() {
      ...
      if (this.state.checkObject['groups']) {
        menuList = this.state.checkObject["groups"].map(function (group, groupIndex) {
          var groupHeader = React.createElement(
            'div',
            null,
            group.group
          );
          ...
          return React.createElement(
            'div',
            { key: groupIndex, style: { paddingBottom: '15px' } },
            groupHeader,
            checkMenuItems
          );
        });
        return React.createElement(
          ...
          React.createElement(
            Well,
            { className: 'fill-height', style: { overflowY: 'scroll' } },
            React.createElement(
              'div',
              null,
              React.createElement(
