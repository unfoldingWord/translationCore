...
const UploadModal = React.createClass({
  ...
  sendFilePath: function (path, link) {
    ...
    if (path) {
      this.loadTranslationStudioManifest(path, function (err, translationStudioManifest) {
        if (err) {
          const alert = {
            title: 'Error Getting Transaltion Studio Manifest',
            content: err.message,
  ...
  render: function () {
    ...
    if (this.state.showFile === true) {
      mainContent = React.createElement(DragDrop, {
      ...
      mainContent = React.createElement(
        ...
        React.createElement(OnlineInput, { ref: "Online", sendFilePath: this.sendFilePath })
      );
    }
    return React.createElement(
      ...
      React.createElement(
        Nav,
        { bsStyle: 'tabs', activeKey: this.state.active, onSelect: this.handleSelect },
        React.createElement(
          NavItem,
          { eventKey: 1 },
          IMPORT_ONLINE
