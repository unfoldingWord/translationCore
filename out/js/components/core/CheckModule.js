...
class CheckModule extends React.Component {
    ...
    if (newGroupIndex !== undefined && newCheckIndex !== undefined) {
      if (newGroupIndex < groups.length && newGroupIndex >= 0) {
        ...
        /* In the case that we're incrementing the check and now we're out of bounds
          ...
          */
        else if (newCheckIndex == groups[currentGroupIndex].checks.length && currentGroupIndex < groups.length - 1) {
  ...
  saveProject() {
    ...
    var commitMessage = 'user: ' + userName + ', namespace: ' + this.nameSpace + ', group: ' + currentGroupIndex + ', check: ' + currentCheckIndex;
