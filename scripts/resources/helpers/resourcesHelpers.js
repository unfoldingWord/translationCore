...
export function generateBibles(bibles, extractedFilePath, RESOURCE_OUTPUT_PATH) {
  console.log('\x1b[33m%s\x1b[0m', "Generating tC compatible resource data structure ...")
  bibles.forEach((bible) => {
    ...
    chapters.forEach(chapterNumber => {
      ...
      if (typeof arrayValue === "number" && !isNaN(arrayValue)) {
        ...
        let bibleId = jsonBibleBook.headers["toc3"].toLowerCase().replace(" ", "");
        console.log(bibleId)
        let savePath = path.join(RESOURCE_OUTPUT_PATH, bibleVersion, bibleId, fileName);
...
function generateBibleManifest(oldManifest, bibleVersion, RESOURCE_OUTPUT_PATH) {
  let newManifest = {};
    ...
    newManifest.description =
      oldManifest.dublin_core.identifier.toLowerCase() === "ulb" || "udb" ? "Gateway Language" : "Original Language";
...
export function getTranslationHelps(extractedFilePath, RESOURCE_OUTPUT_PATH) {
  ...
  const filesPath = path.join(extractedFilePath, 'bible', 'kt');
  const resourceVersion = 'v' + resourceManifest.dublin_core.version;
  const files = fs.readdirSync(filesPath);
  
  files.forEach((fileName) => {
    const sourcePath = path.join(filesPath, fileName);
    const destinationPath = path.join(RESOURCE_OUTPUT_PATH, resourceVersion, 'content', fileName);
