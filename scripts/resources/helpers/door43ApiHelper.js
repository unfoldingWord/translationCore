...
export const getUrl = async (languageId, resourceId) => {
  ...
  let url;
  let languageResources = response.data.languages.filter(language => { // filter languages to match languageId
    return language.identifier === languageId;
  });

  let resource = languageResources[0].resources.filter(resource => { // filter resources to match resourceId
