...
if(!process.argv[2] || !process.argv[3]) {
  console.error('\x1b[31m%s\x1b[0m', "Please specify the resource Id and/or language Id")
  console.error('\x1b[36m%s\x1b[0m', "For example: npm run update-resource ulb en,\n where 'ulb' is the resource Id and 'en' the language Id")
  process.exit(1);
}
...
const RESOURCE_OUTPUT_PATH = path.join(__dirname, "../../static", 'resources', LANGUAGE_ID, RESOURCE_TYPE, RESOURCE_Key);
...
fs.mkdirsSync(RESOURCE_OUTPUT_PATH);
  ...
  console.log('\x1b[36m%s\x1b[0m', "The following bible resource will be updated")
  console.log('\x1b[35m%s\x1b[0m', RESOURCE_ID);

  door43ApiHelper.getUrl(LANGUAGE_ID, RESOURCE_ID).then((url) => {
    axios({
      method:'get',
      url: url,
      responseType:'stream'
    ...
    .then((response) => {
      ...
      response.data.on('end', () => {
        ...
        const extractedFileName = fs.readdirSync(RESOURCE_INPUT_PATH)[0];
        const extractedFilePath = path.join(RESOURCE_INPUT_PATH, extractedFileName);

        if (RESOURCE_TYPE === 'bibles') {  
          // get array of bibles
          const bibles = fs.readdirSync(extractedFilePath).filter(file => { // filter the files to only use .json
            return path.extname(file) === '.usfm';
          });
          // generate bible package
          resourcesHelpers.generateBibles(bibles, extractedFilePath, RESOURCE_OUTPUT_PATH);
        } else {
          resourcesHelpers.getTranslationHelps(extractedFilePath, RESOURCE_OUTPUT_PATH);
        }
      });
    })
    .then(() => {
