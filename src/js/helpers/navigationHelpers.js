...
export const shiftGroupIndex = (_shift, contextId, groupsIndex, groupsData=[], filters={}) => {
  const currentIndex = groupsIndex.findIndex( (groupIndex) => {
  ...
  while (shiftedIndex != currentIndex) {
    ...
    if (filters && groupsData && (! newGroupData || ! groupIsVisible(newGroupData, filters))) {
      ...
      if (shiftedIndex < 0)
        ...
        shiftedIndex = 0; // don't go past the last, set index to first group
    } else {
 ...
 * @param {*} filters 
 */
export const visibleGroupItems = (groupData, filters={}) => {
  return groupData.filter(item => groupItemIsVisible(item, filters));
};
