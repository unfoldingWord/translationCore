...
class UnlockedLiteralBibleFetcher {
			...
			function(res) {
			    ...
			    res.on('data', 
			    	...
			    	function() {
			            ...
			            _this.assignZipFile(zip);
			            if (assignCallback) {
			            	assignCallback(_this.ulbZipFile);
			            }
				    }	
			    );
		    }
	...
	getBookUSFM(bookAbr) {
		if (this.ulbZipFile) {
			var zipEntries = this.ulbZipFile.getEntries();

			for (var zipEntry of zipEntries) {
				if (zipEntry.name.toLowerCase().indexOf(bookAbr.toLowerCase()) != -1) {
					console.log('Returning BookFile');
